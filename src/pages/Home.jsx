  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-3xl p-6 ep-slide-in"
        style={{ background: "linear-gradient(135deg, rgba(10,15,30,0.98), rgba(4,8,16,0.98))", border: "1px solid rgba(6,182,212,0.3)" }}>
        {/* Logo */}
        <div className="text-center mb-6">
          <div className="text-3xl mb-1">🌍</div>
          <h2 className="text-white font-black text-xl">
            {mode === "login" ? "Welcome Back" : mode === "signup" ? "Found Your Nation" : "Reset Password"}
          </h2>
          <p className="text-slate-500 text-xs mt-1">
            {mode === "login" ? "Sign in to your nation" : mode === "signup" ? "Begin your civilization" : "We'll send you a reset link"}
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === "signup" && (
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Leader Name"
              required className="w-full ep-input px-4 py-3 text-sm text-white placeholder-slate-500"
              style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12 }} />
          )}
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email"
            required className="w-full px-4 py-3 text-sm text-white placeholder-slate-500"
          ...
          {mode !== "forgot" && (
            <div className="relative">
              <input type={showPw ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)}
                ...
                className="w-full px-4 py-3 pr-10 text-sm text-white placeholder-slate-500"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, outline: "none" }} />
          ...
          <button type="submit" disabled={loading}
            ...
            style={{ background: "linear-gradient(135deg, #06b6d4, #8b5cf6)", color: "white", opacity: loading ? 0.7 : 1 }}>
            {loading ? "Please wait…" : mode === "login" ? "Enter the World" : mode === "signup" ? "Found My Nation" : "Send Reset Link"}
          </button>
        </form>

        <div className="mt-4 text-center space-y-2">
          {mode === "login" && (<>
            <button onClick={() => { setMode("forgot"); setMsg(null); }} className="text-xs text-slate-500 hover:text-cyan-400 transition-colors">Forgot password?</button>
            <div className="text-xs text-slate-600">
              No account? <button onClick={() => { setMode("signup"); setMsg(null); }} className="text-cyan-400 hover:text-cyan-300 font-bold">Sign up free</button>
            </div>
          </>)}
          {mode === "signup" && (
            <div className="text-xs text-slate-600">
              Already a leader? <button onClick={() => { setMode("login"); setMsg(null); }} className="text-cyan-400 hover:text-cyan-300 font-bold">Log in</button>
            </div>
          )}
          {mode === "forgot" && (
            <button onClick={() => { setMode("login"); setMsg(null); }} className="text-xs text-slate-500 hover:text-cyan-400 transition-colors">Back to login</button>
          )}
        </div>
      </div>
    </div>
    ...
    <div style={{ background: "#040810", fontFamily: "Inter, sans-serif", minHeight: "100vh", color: "white" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 px-6 py-3 flex items-center justify-between"
        ...
        <div className="hidden md:flex items-center gap-6 text-xs font-bold text-slate-400">
          {[["Features", "#features"], ["Roadmap", "#roadmap"], ["About", "#about"], ["Support", "#support"]].map(([label, href]) => (
