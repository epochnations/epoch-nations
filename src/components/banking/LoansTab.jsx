...
const LOAN_TYPES = {
  short_term: {
    label: "Short-Term Emergency", emoji: "⚡",
    description: "Emergency funds. Very high interest, short repayment window.",
    interest_rate: 18, duration_days: 30, max_multiplier: 0.15, color: "red",
  },
  development: {
    label: "Development Loan", emoji: "🏗️",
    description: "For infrastructure, city expansion, and industry. Medium interest.",
    interest_rate: 8, duration_days: 365, max_multiplier: 0.35, color: "cyan",
  },
  sovereign_bond: {
    label: "Sovereign Bond", emoji: "📜",
    description: "Large national projects. Investors can purchase. Long-term interest payments.",
    interest_rate: 5, duration_days: 730, max_multiplier: 0.60, color: "violet",
  },
};
...
function getDebtRatio(loans, nw) {
  const totalDebt = loans.filter(l => l.status === "active").reduce((s, l) => s + (l.balance_remaining || 0), 0);
...
const SUB_TABS = ["apply", "my_loans", "debt_market"];
...
export default function LoansTab({ nation, onRefresh }) {
  ...
  const maxAllowed  = debtRatio < 0.7;
  ...
  async function applyLoan() {
    ...
    await base44.entities.Loan.create({
      borrower_nation_id: nation.id, borrower_nation_name: nation.name,
      borrower_email: nation.owner_email, loan_type: loanType, purpose: loanPurpose,
      principal: amount, balance_remaining: totalRepayable, interest_rate: def.interest_rate,
      interest_accrued: 0, repayment_per_tick: parseFloat(perTick.toFixed(6)),
      issued_at: now.toISOString(), due_date: due.toISOString(),
      status: "active", is_defaulted: false, investor_premium: 10,
    ...
    await base44.entities.Nation.update(nation.id, { currency: (nation.currency || 0) + amount });
  ...
  async function repayLoan(loan, partial) {
    setRepaying(loan.id);
    ...
    setRepaying(null); await load(); onRefresh?.();
  }

  async function buyDebt(loan) {
    setBuying(loan.id);
    const cost = loan.balance_remaining * 0.7;
    ...
    await Promise.all([
      base44.entities.Loan.update(loan.id, {
        status: "active", creditor_nation_id: nation.id, creditor_nation_name: nation.name,
        creditor_email: nation.owner_email, interest_rate: investorRate, is_defaulted: false,
      }),
      base44.entities.Nation.update(nation.id, { currency: Math.max(0, (nation.currency || 0) - cost) }),
    ]);
    setBuying(null); await load(); onRefresh?.();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-center">
            <div className="text-[10px] text-slate-500 ep-mono uppercase">{s.label}</div>
            <div className={`font-bold text-sm ep-mono mt-0.5 ${s.color}`}>{s.val}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-1 border-b border-white/10">
        {[
          ...
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={`px-4 py-2.5 text-xs font-bold border-b-2 transition-all ${subTab===t.id ? "border-violet-400 text-violet-400" : "border-transparent text-slate-500 hover:text-slate-300"}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {!maxAllowed && (
            <div className="flex items-center gap-2 p-3 rounded-xl border border-red-500/30 bg-red-500/10 text-red-400 text-xs">
              <AlertTriangle size={13} /> Debt ratio too high ({(debtRatio*100).toFixed(0)}%). Repay existing loans first.
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            {loanAmount > 0 && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-xs space-y-1">
                <div className="text-slate-400 font-bold mb-1">Repayment Summary</div>
                ...
                <div className="flex justify-between font-bold border-t border-white/10 pt-1"><span className="text-white">Total Repayable</span><span className="text-amber-400 ep-mono">{(parseFloat(loanAmount||0)*(1+LOAN_TYPES[loanType].interest_rate/100)).toLocaleString(undefined,{maximumFractionDigits:0})} cr</span></div>
            ...
            <button onClick={applyLoan} disabled={applying || !maxAllowed || !loanAmount}
              className="w-full py-3 rounded-xl text-sm font-bold text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
      ...
      {subTab === "my_loans" && (
        <div className="space-y-3">
          {activeLoans.length === 0 && <div className="text-center py-10 text-slate-500 text-sm">No active loans.</div>}
          {activeLoans.map(loan => {
            ...
            return (
              <div key={loan.id} className={`rounded-xl border p-4 ${isOverdue ? "border-red-500/40 bg-red-500/5" : `${c.border} ${c.bg}`}`}>
                ...
                {loan.creditor_nation_name && <div className="text-xs text-amber-400 mb-2">📋 Creditor: {loan.creditor_nation_name}</div>}
                <div className="flex gap-2">
                  <button onClick={() => repayLoan(loan, Math.min(loan.balance_remaining, nation.currency||0))}
                  ...
                  <button onClick={() => repayLoan(loan, Math.min(loan.balance_remaining * 0.1, nation.currency||0))}
                    disabled={repaying === loan.id || (nation.currency||0) <= 0}
      ...
      {subTab === "debt_market" && (
        <div className="space-y-3">
          ...
          {debtMarket.length === 0 && <div className="text-center py-10 text-slate-500 text-sm">No defaulted debt available.</div>}
          {debtMarket.map(loan => {
            ...
            return (
              <div key={loan.id} className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
                ...
                <button onClick={() => buyDebt(loan)} disabled={buying === loan.id || !canAfford}
                  className="w-full py-2.5 rounded-xl text-xs font-bold text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
