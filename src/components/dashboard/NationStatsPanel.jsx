...
function ResourceCell({ label, resKey, value }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] text-slate-400 ep-mono font-bold">{label}</span>
      <span className="text-[14px] font-black ep-mono" style={{ color }}>
        {fmtRes(displayVal)}
      </span>
      {warehouseOverflow > 0 && (
...
export default function NationStatsPanel({ nation }) {
  ...
  const stability    = Math.min(100, Math.max(0, Math.floor(nation.stability || 75)));
  ...
  return (
    <div
      className="flex flex-col gap-2 p-3 rounded-2xl h-full overflow-hidden justify-between"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[12px] text-slate-400 truncate">{nation.leader}</div>
          <div className="text-[12px] font-bold" style={{ color: "#22d3ee" }}>{nation.epoch}</div>
        </div>
      </div>
      ...
      <div className="shrink-0">
        <div className="flex justify-between text-[12px] mb-1">
          <span className="text-slate-400 ep-mono font-bold">EPOCH PROGRESS</span>
        ...
        <div className="flex justify-between text-[11px] mt-0.5 text-slate-500 ep-mono">
      ...
      <div className="grid grid-cols-3 gap-1.5 shrink-0">
        ...
        <div className="rounded-xl p-2 flex flex-col items-center gap-0.5"
          ...
          <span className="text-[11px] text-slate-400 font-bold ep-mono uppercase">Allies</span>
        ...
        <div className="rounded-xl p-2 flex flex-col items-center gap-0.5"
          ...
          <span className="text-[11px] text-slate-400 font-bold ep-mono uppercase">Cities</span>
        ...
        <div className="rounded-xl p-2 flex flex-col items-center gap-0.5"
          ...
          <span className="text-[11px] text-slate-400 font-bold ep-mono uppercase">Pop</span>
      ...
      <div className="rounded-xl px-3 py-2 shrink-0"
        ...
        <div className="flex items-center justify-between mb-1.5">
          <div>
            <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase">Treasury</div>
            <FlashStat value={Math.round(nation.currency || 0)} className="text-[16px] font-black ep-mono text-amber-400">
              {Math.round(nation.currency || 0).toLocaleString()}
            </FlashStat>
            <div className="text-[11px] text-amber-600 ep-mono">{nation.currency_name || "Credits"}</div>
          </div>
          <div className="text-right">
            <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase">GDP</div>
            <FlashStat value={Math.round(nation.gdp || 0)} className="text-[16px] font-black ep-mono text-green-400">
              {Math.round(nation.gdp || 0).toLocaleString()}
            </FlashStat>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-1.5 border-t border-amber-500/10">
          <span className="text-[11px] text-slate-400 font-bold ep-mono uppercase">Stock Index</span>
          <span className="text-[14px] font-black ep-mono text-cyan-400 ep-glow-cyan">
      ...
      <div className="shrink-0">
        <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase mb-1">NATURAL RESOURCES</div>
      ...
      <div className="rounded-xl px-3 py-2 shrink-0" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="flex items-center justify-between mb-1">
          <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase">ECONOMY / MIN</div>
          <div className="text-[10px] text-slate-500 ep-mono">1 tick = 1 min</div>
        </div>
        <div className="grid grid-cols-3 gap-1 text-center">
          ...
          <div>
            <div className="text-[11px] text-slate-400">Expense</div>
      ...
      <div className="rounded-xl px-3 py-2 shrink-0" style={{ background: "rgba(74,222,128,0.04)", border: "1px solid rgba(74,222,128,0.10)" }}>
        <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase mb-1">FOOD / MIN</div>
        <div className="grid grid-cols-3 gap-1 text-center">
          <div>
            <div className="text-[11px] text-slate-400">Produce</div>
            <div className="text-[14px] font-black ep-mono text-green-400">+{foodProd}</div>
          </div>
          <div>
            <div className="text-[11px] text-slate-400">Consume</div>
      ...
      <div className="rounded-xl px-3 py-2 shrink-0" style={{ background: "rgba(251,146,60,0.05)", border: "1px solid rgba(251,146,60,0.12)" }}>
        <div className="flex items-center justify-between mb-1">
          <div className="text-[11px] text-slate-400 font-bold ep-mono uppercase">HOUSING CAPACITY</div>
        ...
        <div className="flex justify-between text-[10px] ep-mono text-slate-500 mt-0.5">
