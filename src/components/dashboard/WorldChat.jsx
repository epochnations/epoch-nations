...
// Default built-in blocked words — base list
...
function containsBlockedWord(text, blockedList) {
  const lower = text.toLowerCase();
  return blockedList.some(w => {
...
// Parse @NationName mention at the start
...
export default function WorldChat({ myNation, user }) {
  ...
  const [privateTarget, setPrivateTarget] = useState(null); // { nationId, nationName, flag, color }
  ...
  // Load custom blocked words from DB
  useEffect(() => {
  ...
  async function handleCommand(raw) {
    ...
    if (cmd === "/promote" && isAdmin) {
      ...
      const newRole = parts[2] || "moderator";
      if (!targetName) return;
      const nations = await base44.entities.Nation.filter({ name: targetName });
      const target = nations[0];
      if (!target) { return; }
      // Find the user record and update their role via base44.entities.User
    ...
    if (cmd === "/sanction" && myNation) {
      ...
      await base44.entities.ChatMessage.create({
        ...
        sender_color: myNation.flag_color || "#3b82f6",
        sender_role: myRole,
        content: `[DIPLOMATIC ACTION] ⛔ ${myNation.name} announces economic sanctions against ${target}. Trade relations suspended.`,
      });
      return;
    }
    ...
    if (cmd === "/aid" && myNation) {
      ...
      await base44.entities.ChatMessage.create({
        ...
        sender_color: myNation.flag_color || "#3b82f6",
        sender_role: myRole,
        content: `[DIPLOMATIC ACTION] 🤲 ${myNation.name} pledges humanitarian aid of ${amt} credits to ${targetNation}.`,
      });
      return;
    }
  ...
  async function sendMessage() {
    ...
    // Check for @NationName private message trigger
    if (trimmed.startsWith("@")) {
      const mentionedName = parsePrivateMention(trimmed);
      if (mentionedName) {
        // Find nation by name to get ID/flag/color
        ...
        if (targetNation) {
          setPrivateTarget({
            ...
            flag: targetNation.flag_emoji || "🏴",
            color: targetNation.flag_color || "#64748b",
          });
          setShowPrivate(true);
          setInput("");
          return;
  ...
  function openPrivateFromMsg(msg) {
    setPrivateTarget({
      nationId: msg.sender_nation_id,
      nationName: msg.sender_nation_name,
      flag: msg.sender_flag || "🏴",
      color: msg.sender_color || "#64748b",
    });
    setShowPrivate(true);
  }
          ...
          <div className="flex gap-1.5 items-center">
            ...
            <input
              ...
              placeholder={myNation
                ? `Message #${channel} or @NationName for DM${isMod ? " or /command" : ""}…`
