        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <input type="range" min={0} max={60} value={edu} onChange={e => setEdu(+e.target.value)}
              className="w-full accent-cyan-400" style={{ minHeight: 28 }} />
          ...
          <div>
            ...
            <input type="range" min={0} max={60} value={mil} onChange={e => setMil(+e.target.value)}
              className="w-full accent-red-400" style={{ minHeight: 28 }} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-white/5 rounded-xl p-3 text-center">
              <div className="text-xs text-slate-500 mb-1">Welfare</div>
              <div className="font-bold font-mono text-sm text-green-400">{Math.max(0, 100 - totalSpending)}%</div>
            </div>
            <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-3 text-center sm:col-auto col-span-2">
              <div className="text-xs text-slate-500 mb-1">Income/min</div>
              <div className="font-bold font-mono text-sm text-cyan-400">+{Math.floor(nation.gdp * 0.05)} cr</div>
            </div>
          </div>

          {totalSpending > 80 && (
            <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-3">
              <div className="text-xs text-amber-400 font-bold">⚠ Spending &gt;80% erodes public trust each minute. Low trust causes stability loss and eventually tech regression.</div>
            </div>
          )}
